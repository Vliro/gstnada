...
if (($USE_SCREAM == 1)); then
    SCREAMTX="$QUEUE !  nadatx name=\"screamtx\" params=\" $NOSUMMARY -forceidr -ect 1  -initrate 2500 -minrate 500  -maxrate 10000 \" ! "
    SCREAMTX_RTCP="rtpbin name=r udpsrc port=$PORT address=$LOCAL_IP ! queue ! nadatx.rtcp_sink nadatx.rtcp_src !  r.recv_rtcp_sink_0 "
else
    SCREAMTX=""
    SCREAMTX_RTCP=""
fi
